...
`<skill-directory>` is the directory containing this `SKILL.md`; this works with both Codex's and Claude Code's shared `skills/` plugin layout. Resolve it to an absolute path before running the command rather than relying on the current working directory. If `python3` is unavailable, try `python`, then `py -3`.
...
# Step 3: supplement
python3 "<skill-directory>/scripts/search.py" "search loading animation" --domain ux
