...
- [Review Coverage](#review-coverage)
  ...
  - [Review Axis Matrix](#review-axis-matrix)
...
|--------|--------|--------|
| No actionable findings | All enabled review axes passed. | completed |
...
<Documented-standard breaches and `possible <smell>` judgement calls. Cite the guidance source or baseline and the reviewed diff evidence.>

### Spec

<Missing, partial, incorrect, or unrequested behavior. Cite the governing requirement and the reviewed diff evidence. Use `No spec available` only when the user confirmed no governing specification exists.>
...
- `Quick Findings Summary`: include every finding, including optional and info items. Add one concise at-a-glance sentence with counts by axis, blocking categories, and the gate reason. Link each `Detail` cell to the finding's stable `#### <ID>: <Title>` heading. When there are no findings, use the no-actionable-findings row. Do not identify a worst finding across axes.
...
- `Review Execution Mode`: use `delegated-lanes` when the review skill spawned one lane per enabled axis, `parent-orchestrated-lanes` when a parent coordinator supplied axis outputs, `single-agent-fallback` when a bounded target was reviewed in one read-only pass, or `blocked` when required inputs/access are missing or the target is too large or risky for the available mode.
...
- `Axis Reports`: preserve separate `Standards` and `Spec` reports. A finding appears in exactly one axis; the detailed queues organize repair work but do not merge or rerank axis results.
- `Review Execution Lanes`: record each enabled axis with lane-level execution as `delegated`, `parent-orchestrated`, `main-thread-fallback`, `skipped`, or `blocked`. Use `skipped` only when the user confirmed no governing specification exists; use `blocked` only for missing inputs/access, failed required lane results, or scope risk.
...
- `Review Axis Matrix`: mark each axis as `pass`, `finding`, `not-applicable`, or `blocked`.
