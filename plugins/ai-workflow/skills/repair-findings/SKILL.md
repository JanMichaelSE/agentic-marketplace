...
Repair explicit, bounded findings from one or more normalized review artifacts or from a `run-workflow` response artifact repair queue. This skill is a repair executor, not a review-cycle orchestrator.
...
Workflow mode uses `.scratch/<feature-slug>/` inputs, including a response artifact and its repair queue when invoked by `run-workflow`, and writes one durable summary:

```text
.scratch/<feature-slug>/repair/<scope-stem>-repair-summary.md
```

Standalone mode uses explicit review artifacts and a concrete repair boundary without requiring `.scratch` context or a response artifact. Return the same structured summary in chat unless the user asks for a file.
...
- One or more source artifacts normalized into repair queue entries. In workflow mode, this may be a `run-workflow` response artifact plus its `Repair Queue`; in standalone mode, this is one or more explicit review artifacts.
...
For workflow mode, also confirm the feature slug, response artifact path when supplied, repair queue entries, and output summary path. Treat execution plans, slices, implementation summaries, refactor summaries, review summaries, response artifacts, and prior repair summaries as read-only inputs.
...
Normalize all supplied source artifacts before editing. For a `run-workflow` response artifact, include only findings listed in `Repair Queue` with response value `QUEUED_FOR_REPAIR` as candidate repair entries. Repair only findings where every condition is true:
...
5. Normalize source artifacts into a single repair queue using the normalization rules.
